...
class DeepQNetwork:
    ...
    def createQNetwork(self,bef_args=None,isTrain=True):
        if isTrain:
            modQ = mx.mod.Module(symbol=self.sym(), data_names=('data',), label_names=('yInput',), context=self.ctx)
    ...
    def learn(self):
        ...
        for i in range(self.batch_size):
            ...
            self.target.forward(mx.io.DataBatch([mx.nd.array(state_batch_temp,self.ctx)],[]))
        ...
        self.Qnet.forward(mx.io.DataBatch([mx.nd.array(state_batch,self.ctx)],[mx.nd.array(q_target, self.ctx)]),is_train=True)
